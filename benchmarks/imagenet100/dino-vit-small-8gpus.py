...
# Use standard DINO forward that now handles different view sizes
...
# Following DINO paper: 1024 total batch size / 8 GPUs = 128 per GPU
# Total effective batch size with multi-crop: 128 * 8 GPUs * 8 views = 8192 images
batch_size = 128
world_size = 8
total_batch_size = batch_size * world_size  # 1024
...
train_dataloader = torch.utils.data.DataLoader(
    ...
    collate_fn=None,  # Use default collate - dino_forward now handles different sizes
)
...
backbone = spt.backbone.from_timm(
    ...
    dynamic_img_size=True,  # Automatically interpolate position embeddings for different sizes
)
...
# Following DINO paper: lr = 0.0005 * (1024 / 256) = 0.002
lr = 0.0005 * (total_batch_size / 256)

module = spt.Module(
    backbone=wrapped_backbone,
    projector=wrapped_projector,
    forward=spt.forward.dino_forward,  # Use standard DINO forward
    ...
    temperature_teacher=0.07,  # Following DINO paper: warmup from 0.04 to 0.07
    warmup_epochs_temperature_teacher=30,
    optim={
        "optimizer": {
            "type": "AdamW",
            "lr": lr,
            "weight_decay": 0.04,  # Will use cosine schedule from 0.04 to 0.4
            "betas": [0.9, 0.999],  # Standard AdamW betas for ViT
        },
        "scheduler": {
            "type": "LinearWarmupCosineAnnealing",
            "total_steps": 300,  # Total epochs (since interval="epoch")
            "peak_step": 10,  # 10 epochs warmup (absolute number since > 1)
        },
        "interval": "epoch",
    },
)
